...
class TestStupidBot:
    ...
    def test_make_move_bot(self) -> None:
        for _ in range(9):
            move = self.bot.make_bot_move(self.free_cages)
...
class TestSmartBot:
    def test_out_isinstance_int(self) -> None:
        ...
        free_cages = [num for num in range(9)]
        for _ in range(9):
            move = bot.make_bot_move(free_cages)
    ...
    def test_4_is_available(self, free_cages: list[int]) -> None:
        ...
        if 4 not in free_cages:
            free_cages.append(4)
        assert bot.make_bot_move(free_cages) == 4
    ...
    def test_get_win_move(self, win_pos: list[int]) -> None:
        ...
        assert bot.make_bot_move([move]) == move
    ...
    def test_get_diagonal_if_available(self, count: int) -> None:
        ...
        for _ in range(count):
            assert bot.make_bot_move([num for num in range(9) if num != 4]) in [0, 2, 6, 8]

    @given(st.integers(min_value=1, max_value=100))
    def test_get_not_diagonal(self, count: int) -> None:
        bot = SmartBot("Bot", "X", [])
        for _ in range(count):
            assert bot.make_bot_move([num for num in range(9) if num not in [0, 2, 4, 6, 8]]) in [1, 3, 5, 7]
